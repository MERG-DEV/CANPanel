...
const rom BYTE    ledGroupLimits[LED_GROUPS] = {MAX_STORAGE_ROAD, HARDCODED_MAX_BUTTON};
...
void clearhardCodedLeds( BYTE eventIndex )

{
    BYTE i,groupid = 0xFF;
    
    for (i=0; i<LED_GROUPS;i++)
        if (eventIndex >= ledGroupLimits[i] && (eventIndex < ledGroupLimits[i+1]))
            groupid = i;
